...
#define TEST_TRANSFER_DATA_IN_BYTE  (3U)
...
int main(void)
{
    ...
    control_config.common_config.trans_mode = spi_trans_read_dummy_write;//spi_trans_write_read_together;//
...
while(1)
{
    prepare_transfer_data();
            // 关键修复1：在每次传输前清空FIFO
        spi_reset_rx_fifo(TEST_SPI);
        spi_reset_tx_fifo(TEST_SPI);
