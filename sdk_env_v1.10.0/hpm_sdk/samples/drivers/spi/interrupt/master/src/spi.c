...
int main(void)
{
    ...
    format_config.common_config.mode = spi_slave_mode;//spi_master_mode;
    ...
    stat = spi_write_address(TEST_SPI, spi_slave_mode, &control_config, &addr);
    if (stat != status_success) {
    ...
    stat = spi_write_command(TEST_SPI, spi_slave_mode, &control_config, &cmd);
    if (stat != status_success) {
